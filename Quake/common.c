...
#define PAK0_COUNT		339	/* lbb/pak0.pak - v1.0x */
#define PAK0_CRC_V100		13900	/* lbb/pak0.pak - v1.00 */
#define PAK0_CRC_V101		62751	/* lbb/pak0.pak - v1.01 */
#define PAK0_CRC_V106		32981	/* lbb/pak0.pak - v1.06 */
#define PAK0_CRC	(PAK0_CRC_V106)
#define PAK0_COUNT_V091		308	/* lbb/pak0.pak - v0.91/0.92, not supported */
#define PAK0_CRC_V091		28804	/* lbb/pak0.pak - v0.91/0.92, not supported */
...
char	com_gamenames[1024];	//eg: "hipnotic;quoth;warp", no lbb, no private stuff
...
//if either contain lbb then that gets ignored
...
static void COM_Game_f (void)
{
	if (Cmd_Argc() > 1)
	{
		...
		*paths = 0;
		...
		for (pri = 0; pri <= 1; pri++)
		{
			for (i = 1; i < Cmd_Argc(); i++)
			{
				...
				if (!q_strcasecmp(p, GAMENAME))
					continue; //don't add lbb, its not interesting enough.
		...
		VID_Lock ();
		Cbuf_AddText ("exec allotropy.rdm\n");
...
void COM_InitFilesystem (void) //johnfitz -- modified based on topaz's tutorial
{
	...
	if (i)
	{	//-basegame:
		// a) replaces all hardcoded dirs (read: alternative to lbb)
		// b) isn't flushed on normal gamedir switches (like lbb).
	...
	{
		// start up with GAMENAME by default (lbb)
