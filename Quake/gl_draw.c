...
void Draw_LoadPics (void)
{
	...
	if (info->size != 128*128)			Con_Warning("Invalid size for gui.wad conchars lump (%u, expected %u) - attempting to ignore for compat.\n", info->size, 128*128);
	else if (info->type != TYP_MIPTEX)	Con_DWarning("Invalid type for gui.wad conchars lump - attempting to ignore for compat.\n"); //not really a miptex, but certainly NOT a qpic.
