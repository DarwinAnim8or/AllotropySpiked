...
{	//okay, so this is silly. we've ended up with 3 different cache levels. qcpics, pics, and images.
	...
	//the extra gui/ crap is because DP insists on it for wad images. and its a nightmare to get things working in all engines if we don't accept that quirk too.
	if (cachetype == 1)
		qcpics[i].pic = Draw_PicFromWad(picname + (strncmp(picname, "gui/", 4)?0:4));
	else if (!strncmp(picname, "gui/", 4) && !strchr(picname+4, '.'))
...
static void PF_getgamedirinfo(void)
{
	...
	case 5:	//lbb;hipnotic;quoth type list
//		G_INT(OFS_RETURN) = 0;
		break;
	}
}
...
//	{"entityprotection",NULL,				NULL,				0,		PF_NoMenu, D("float(entity e, float nowreadonly)", "Changes the protection on the specified entity to protect it from further edits from QC. The return value is the previous setting. Note that this can be used to unprotect the world, but doing so long term is not advised as you will no longer be able to detect invalid entity references. Also, world is not networked, so results might not be seen by clients (or in other words, world.avelocity_y=64 is a bad idea).")},
	...
	{"whichpack",		PF_whichpack,		PF_whichpack,		503,	PF_whichpack,503, D("string(string filename, optional float makereferenced)", "Returns the pak file name that contains the file specified. models/player.mdl will generally return something like 'pak0.pak'. If makereferenced is true, clients will automatically be told that the returned package should be pre-downloaded and used, even if allow_download_refpackages is not set.")},//DP_QC_WHICHPACK
