...
/*	if (sv.csqc_progsize)
	...
	if (client->protocol_pext2 & PEXT2_PREDINFO)
	{
		//if multiple gamedirs were used, we should list all the active ones eg: "lbb;hipnotic;rogue;quoth;mod".
		//fixme: engine-specific forced gamedirs like lbb/ or qw/ or fte/ are redundant, so don't bother listing them
...
// serverflags are for cross level information (sigils)
	...
	SV_Precache_Model("models/player.mdl");	//Spike -- SV_CreateBaseline depends on this model.
